...
for k in range(len(time_error)):
    std_scalar_pos = pos_error[0]
    std_scalar_time = time_error[k]
    avg_ratio = 0.0 
    avg_smart_ratio = 0.0
    avg_trust_ratio = 0.0
    trust_ratio = 0.0
...
plt.title("Model 1 $\epsilon_{time}$", fontsize = 32)
plt.savefig("Model 1 Time Error",dpi=300, bbox_inches = "tight")  
#plt.show()  
plt.close()
